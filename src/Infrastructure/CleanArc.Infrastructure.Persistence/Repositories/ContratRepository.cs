...
namespace CleanArc.Infrastructure.Persistence.Repositories
{
    internal class ContratRepository : BaseAsyncRepository<T_CONTRAT>, IContratRepository
    {
        ...
        public ContratRepository(ApplicationDbContext dbContext, GetAllContratsQueryHandler contractDetailsQueryHandler) : base(dbContext)
        {
            _dbContext = dbContext;
            _contractDetailsQueryHandler = contractDetailsQueryHandler;
        }
      ...
      /*  public async Task<PagedList<T_CONTRAT>> GetAllContratsAsync(PaginationParams paginationParams)
        {
            ...
            var contracts = query.Skip((paginationParams.PageNumber - 1) * paginationParams.PageSize)
      ...
      public async Task<PagedList<ListeDesContrats_Result>> GetAllContratAsync(PaginationParams paginationParams)
      {
        ...
        /*public async Task<ContractDetailsDTO> GetContractDTOAsync()
        {
            try
            {
                // Appel de la méthode du gestionnaire de requêtes
                return await _contractDetailsQueryHandler.GetContractDetailsAsync();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error getting ContractDTO: {ex.Message}");
                throw;
            }
        }*/
