...
{
  internal class UpdateContratCommandHandler : IRequestHandler<UpdateContratCommand, OperationResult<bool>>
    {
        ...
        private async Task UpdateDemandeLimiteAsync(List<T_DEM_LIMITE_DTO> demandeLimites, int refContract)
        {
            try
            {
                ...
                {
                    var currentDemandeLimite = await _unitOfWork.LimiteRepository.GetDemandeLimiteByRefCtrAndType(refContract, demandeLimite.TYP_DEM_LIM);
                    if (currentDemandeLimite != null)
                    {
                        ...
                        await _unitOfWork.LimiteRepository.UpdateDemandeLimiteAsync(currentDemandeLimite.REF_CTR_DEM_LIM, currentDemandeLimite);
                    }
                }
            }
            catch (Exception e)
        ...
        private async Task UpdateDetAssAsync(T_DET_ASS_DTO detAssDto, int refContract)
        {
            try
            {
                ...
                if (existingDetAss != null)
                {
                    ...
                    await _unitOfWork.DetAssRepository.UpdateDetAssAsync(existingDetAss.REF_ASS, existingDetAss);
                }
                else
                {
