...
namespace CleanArc.Application.Features.FraisDivers.Queries.GetByIdQuery
{
    internal class GetByIdFraisDiversQueryHandler : IRequestHandler<GetByIdFraisDiversQuery, OperationResult<GetByIdFraisDiversQueryResult>>
    {
        ...
        {
            var fraisDivers = await _unitOfWork.FraisDiversRepository.GetFraisDiversByRefCtrAndType(request.fraisDiversId, request.fraisDiversType);
            ...
            var result = _mapper.Map<T_FRAIS_DIVER, GetByIdFraisDiversQueryResult>(fraisDivers);
